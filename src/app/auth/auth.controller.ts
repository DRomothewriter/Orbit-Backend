	...
	try {
		...
		if (!user.isVerified) {
			return res.status(Status.FORBIDDEN).json({
				error:
					'Email no verificado. Por favor verifica tu correo antes de iniciar sesión.',
			});
		}
	...
	try {
		...
		if (!user) {
			user = await User.create({
				...
				isVerified: true,
				//authProvider: 'google' | 'github' //Esto lo necesitaremos en el futuro
			});
		} else {
			if (!user.isVerified) {
			...
			if (!user.profileImgUrl && payload?.picture) {
				user.profileImgUrl = payload?.picture;
				await user.save();
			}
		}
...
export const signup = async (req: Request, res: Response) => {
	...
	try {
		...
		if (exists)
			return res.status(Status.CONFLICT).json({ error: 'User already exists' });
...
export const resendVerificationCode = async (req: Request, res: Response) => {
	...
	try {
		...
		if (user.isVerified) {
			return res
				.status(Status.BAD_REQUEST)
				.json({ error: 'El email ya está verificado' });
		}
